...
| **트램펄린 벽** | 2.0  | 0.05 | 15 HP | 공을 강하게 튕김, 충돌 시 찌그러짐 <br>애니메이션    |       |
...
#### 기본 스테이지 (10개)
- `public/stages/stage-01.json` ~ `stage-10.json`에 개별 파일로 저장
- `public/stages/index.json`에 파일 목록 관리
- 플레이 모드 진입 시 fetch로 로딩, level 순 정렬
- 진행도는 localStorage (`justball_stageProgress`)에 클리어한 최고 level 저장
...
- 저장된 맵은 로컬 스토리지에 보관, 파일 내보내기/가져오기로 브라우저 간 공유 가능
